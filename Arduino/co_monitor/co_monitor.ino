/* NNR: This code runs on Arduino Nano, ATMega328P (Old bootleader) */
...
void setTimer0PWM(byte chA, byte chB) //pins D5 and D6
{
	...
	TCCR0B = 0b010; //8 prescaler - instead of system'serialPort default 64 prescaler - thus time moves 8 times faster
	OCR0A = chA; //0..255
	OCR0B = chB;
}
...
float opt_voltage = 3.62; //default = 0;
byte opt_width = 48; //default=240; //default reasonable value
...
//this value is used in the program later
{
	...
	float raw2v = 5.0 / 1024.0;//coefficient to convert Arduino'serialPort 
...
float sensor_reading_clean_air = 682;   //measured after 60 hours constant on, last 10h in clean air. Fill raw sensor value at the end of measurement phase (before heating starts) in clean air here! That is critical for proper calculation
...
float cur_CO_ppm_measurement = 0; //CO concentration currently
...
float raw_value_to_CO_ppm(float value)
{
  ...
  Serial.print("sensor_100ppm_CO_resistance_kOhm="); Serial.println(sensor_100ppm_CO_resistance_kOhm);
	float sensor_R_kOhm = reference_resistor_kOhm * 1023 / value - reference_resistor_kOhm;
  Serial.print("sensor_R_kOhm="); Serial.println(sensor_R_kOhm);
	float R_relation = sensor_100ppm_CO_resistance_kOhm / sensor_R_kOhm;
  Serial.print("R_relation="); Serial.println(R_relation);
...
void SendJsonOnSerial(float cur_CO_ppm_measurement, float last_CO_ppm_measurement, float sens_val, byte phase, int i, int j) {
	StaticJsonBuffer<150> jsonBuffer;
	char jsonChar[150];
	
	JsonObject& root = jsonBuffer.createObject();
	root["cur_ppm"] = cur_CO_ppm_measurement;
	root["last_ppm"] = last_CO_ppm_measurement;
	root["sens_val"] = sens_val;
	root["phase"] = phase;
	// include i and j for testing with TestSerialComm
	//root["i"] = i;
	//root["j"] = j;
//	Serial.println(" ");
...
//	Serial.println("Sending data ... ");
	serialPort.write("<");
	for (int i = 0; i < root.measureLength() + 1; i++) {
		serialPort.write(jsonChar[i]);
		Serial.write(jsonChar[i]);
	}
	serialPort.write("\0");
//	Serial.write("\0");
	serialPort.write(">");
	//	serialPort.println(jsonChar);
...
void EmptySerialReadBuffer() {
	char c;
	Serial.print("Empty read buffer: ");
...
//	Serial.println("Data output: raw A0 value/100, heating on/off (0/1), CO ppm from last measurement cycle");
...
void loop()
{
	...
	//than 0.1 seconds - but that'serialPort fine, since it happens only once
	//per several days
	if (dt > 100 * time_scale || dt < 0)
	{
		prev_ms = ms; //store previous cycle time
		sec10++; //increase 0.1serialPort counter
	...
	if (sec10 - last_print > 39) //print measurement result into serial 2 times per second
	{
	    cur_CO_ppm_measurement = raw_value_to_CO_ppm(sens_val);
		last_print = sec10;
/*		Serial.print(sens_val/100);
		Serial.print(" ");
		Serial.print(phase);
		Serial.print(" ");
		Serial.println(last_CO_ppm_measurement);
		Serial.print(" ");
*/		Serial.println(cur_CO_ppm_measurement);
	}

	if (serialPort.available() > 0)
	{
		Serial.println("Sensing something");
		if (serialPort.read() == 'X') {
			Serial.println("Got the X ");
			SendJsonOnSerial(cur_CO_ppm_measurement, last_CO_ppm_measurement, sens_val, phase, i, j++);
		}
	}
